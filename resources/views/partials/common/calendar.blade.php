...
@once
    @push('styles')
        ...
        <link rel="stylesheet"
            href="{{ asset('css/partials/calendar.css') }}?v={{ filemtime(public_path('css/partials/calendar.css')) }}">
    @endpush
...
<div class="flex justify-center ">
    <div class="relative min-w-[240px] w-full">
        ...
        <label for="{{ $id }}"
            ...
            data-calendar-label>
            <button type="button" class="absolute right-[10px] flex items-center justify-center w-8 h-8 rounded-lg border border-[#D92D20] bg-white hover:bg-red-50 z-10 clear-calendar-btn {{ $value ? '' : 'hidden' }}" data-calendar-clear title="مسح التاريخ">
                <iconify-icon icon="mingcute:close-fill" width="20" height="20" class="text-[#D92D20]"></iconify-icon>
            </button>
